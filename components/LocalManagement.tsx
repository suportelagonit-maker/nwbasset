...
export default function LocalManagement({ empresaId }: { empresaId: number | null }) {
  ...
  useEffect(() => {
    void Promise.all([loadLocais(), loadFiliais(), loadUnidades(), loadDepartamentos()]);
  }, []);
  ...
  const departamentosDisponiveis = useMemo(
    () =>
      departamentos.filter((departamento) => {
        if (form.filial_id && departamento.filial_id !== Number(form.filial_id)) {
          return false;
        }

        if (form.unidade_administrativa_id && departamento.unidade_administrativa_id !== Number(form.unidade_administrativa_id)) {
          return false;
        }
  ...
  async function loadLocais() {
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as { data?: LocalItem[]; message?: string } | null;
      ...
      if (!response.ok) {
        throw new Error(payload?.message ?? `Falha ao consultar locais: ${response.status}`);
      }

      setLocais(payload?.data ?? []);
    } catch (fetchError) {
      setError(fetchError instanceof Error ? fetchError.message : 'NÃ£o foi possÃ­vel consultar os locais.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function loadDepartamentos() {
    try {
      ...
      const payload = (await response.json().catch(() => null)) as { data?: DepartamentoItem[] } | null;
  ...
  function updateField<K extends keyof LocalFormState>(field: K, value: LocalFormState[K]) {
    setForm((current) => {
      if (field === 'filial_id') {
        return {
          ...current,
  ...
  async function handleSubmit() {
    const endpoint = modalMode === 'edit' && selectedLocal ? `/api/locais/${selectedLocal.id}` : '/api/locais';
    ...
    if (!response.ok || !payload?.data) {
      ...
      setError(firstError ?? payload?.message ?? 'NÃ£o foi possÃ­vel salvar o local.');
      return;
    }

    setMessage(modalMode === 'edit' ? 'Local atualizado com sucesso.' : 'Local cadastrado com sucesso.');
    resetModal();
    await loadLocais();
  }
  ...
  async function handleDelete(local: LocalItem) {
    setConfirmDialog({
      ...
      action: async () => {
        ...
        if (!response.ok) {
          setError(payload?.message ?? 'NÃ£o foi possÃ­vel excluir o local.');
          return;
        }
  ...
  function getDepartamentoNome(departamentoId: number) {
    return departamentos.find((departamento) => departamento.id === departamentoId)?.nome ?? `Departamento #${departamentoId}`;
  }
    ...
    <>
      ...
      <section className="panel-surface rounded-[28px] p-5 md:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-[12px] font-semibold uppercase tracking-[0.16em] text-[var(--muted)]">Locais</p>
            <h2 className="mt-1.5 text-[1.6rem] font-semibold tracking-[-0.045em] text-[var(--ink)]">Locais fÃ­sicos de alocaÃ§Ã£o</h2>
            <p className="mt-2.5 max-w-3xl text-[13px] leading-6 text-[var(--muted)]">
              O local representa o ponto fÃ­sico onde o bem fica alocado, sempre abaixo de filial, unidade administrativa e departamento.
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-3">
            <button type="button" onClick={openCreateModal} className="inline-flex h-10 items-center justify-center rounded-full bg-[var(--accent)] px-4.5 text-[13px] font-semibold text-white transition hover:opacity-90">
              Novo local
            </button>
            <button type="button" onClick={() => void Promise.all([loadLocais(), loadFiliais(), loadUnidades(), loadDepartamentos()])} className="inline-flex h-10 items-center justify-center rounded-full border border-[var(--line)] bg-white px-4.5 text-[13px] font-semibold text-[var(--ink)] transition hover:border-[var(--accent)] hover:text-[var(--accent)]">
              Atualizar
            </button>
          </div>
            ...
            <table className="min-w-full border-collapse">
              ...
              <tbody>
                {loading ? (
                  ...
                  locais.map((local) => (
                    <tr key={local.id} className="border-t border-[var(--line)] text-sm text-[var(--ink)]">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-xs text-[var(--muted)]">
                              ID {local.id} {local.endereco ? `| ${local.endereco}` : '| Sem endereco informado'}
                            </p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-4">{local.codigo}</td>
                      <td className="px-4 py-4">{getDepartamentoNome(local.departamento_id)}</td>
                      <td className="px-4 py-4">
                        <p>{getFilialNome(local.filial_id)}</p>
                        <p className="mt-1 text-xs text-[var(--muted)]">{getUnidadeNome(local.unidade_administrativa_id)}</p>
                      </td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <button type="button" onClick={() => openViewModal(local)} className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-[var(--line)] bg-white text-[var(--ink)] transition hover:border-[var(--accent)] hover:text-[var(--accent)]" aria-label="Visualizar local" title="Visualizar local">
                            <EyeIcon />
                          </button>
                          <button type="button" onClick={() => openEditModal(local)} className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-[var(--line)] bg-white text-[var(--ink)] transition hover:border-[var(--accent)] hover:text-[var(--accent)]" aria-label="Editar local" title="Editar local">
                            <PencilIcon />
                          </button>
                          <button type="button" onClick={() => void handleDelete(local)} className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-[rgba(190,18,60,0.15)] bg-white text-[var(--rose)] transition hover:border-[rgba(190,18,60,0.35)] hover:bg-[rgba(190,18,60,0.06)]" aria-label="Excluir local" title="Excluir local">
                            <TrashIcon />
                          </button>
                        </div>
                      </td>
            ...
            <div className="flex flex-col gap-3 border-b border-[var(--line)] pb-3 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <h3 className="admin-modal-title">
                  {modalMode === 'create' ? 'Novo local' : modalMode === 'edit' ? 'Editar local' : 'Visualizar local'}
                </h3>
                <p className="mt-1.5 max-w-2xl text-[11px] leading-5 text-[var(--muted)]">
                  O local depende da hierarquia organizacional e usa codigo gerado automaticamente pelo backend.
                </p>
              </div>
            ...
            <section className="admin-step-shell">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-[11px] leading-5 text-[var(--muted)]">Monte o local dentro da estrutura correta de filial, unidade e departamento.</p>
                </div>
            ...
            <div className="mt-3">
              <div className="admin-modal-block">
                <div className="admin-modal-form-grid md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <label className="admin-field">
                    Departamento
                    <select value={form.departamento_id} onChange={(event) => updateField('departamento_id', event.target.value)} disabled={isReadOnly || !form.unidade_administrativa_id} className="admin-select">
                  ...
                  <label className="admin-field">
                    Codigo do cadastro
                    <input type="text" value={selectedLocal?.codigo ?? ''} disabled placeholder="Gerado automaticamente ao salvar" className="admin-input" />
                  </label>
                  ...
                  <label className="admin-field md:col-span-2 xl:col-span-3">
                    Descricao
                    <textarea value={form.descricao} onChange={(event) => updateField('descricao', event.target.value)} disabled={isReadOnly} rows={3} className="admin-textarea" />
                  </label>
                </div>
              </div>
            </div>
            ...
            {!isReadOnly ? (
              <div className="admin-modal-footer -mx-3 px-3 md:-mx-3.5 md:px-3.5">
                ...
                <button type="button" onClick={() => void handleSubmit()} className="admin-btn-primary">
                  {modalMode === 'edit' ? 'Salvar alteracoes' : 'Cadastrar local'}
                </button>
                </div>
              </div>
            ) : null}
