...
export default function GenericModuleManagement({
  ...
  const [tipoBemFiltro, setTipoBemFiltro] = useState<string>("");
  ...
  const tipoBemFiltroOptions = useMemo(() => {
    if (!isTipoProdutoModule) return [];
    const map = new Map<string, { value: string; label: string }>();
    const push = (value: string, label: string) => {
      const normalized = normalizeSelectText(label || value);
      if (!normalized) return;
      if (map.has(normalized)) return;
      map.set(normalized, { value, label });
    };
    (lookups.tipos_bens ?? []).forEach((tipo) => {
      const value = String(tipo.id ?? "");
      const label =
        getLookupLabel("tipos_bens", tipo.id) ||
        String(tipo.nome ?? tipo.tipo_bem ?? "");
      push(value || label, label);
    });
    records.forEach((record) => {
      const raw =
        String(record.tipo_bem ?? record.tipo ?? record.tipo_nome ?? "") || "";
      if (raw) {
        push(raw, raw);
      }
    });
  ...
  }, [getLookupLabel, isTipoProdutoModule, lookups.tipos_bens, records]);
  const filteredRecords = useMemo(() => {
    if (!isTipoProdutoModule || !tipoBemFiltro) return records;
    const alvo = normalizeSelectText(tipoBemFiltro);
    return records.filter((record) => {
      const label =
        getLookupLabel("tipos_bens", record.tipo_bem_id) ||
        String(record.tipo_bem ?? record.tipo ?? record.tipo_nome ?? "");
      const matchLabel = normalizeSelectText(label);
      const matchId = normalizeSelectText(String(record.tipo_bem_id ?? ""));
      return matchLabel === alvo || matchId === alvo;
    });
  }, [getLookupLabel, isTipoProdutoModule, records, tipoBemFiltro]);
  ...
  async function handleDelete(record: Record<string, unknown>) {
    setConfirmDialog({
      ...
      description:
        "Essa a\u00e7\u00e3o remove o registro selecionado do sistema. Deseja continuar?",
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {isTipoProdutoModule ? (
              <select
                value={tipoBemFiltro}
                onChange={(event) => setTipoBemFiltro(event.target.value)}
                className="h-10 min-w-[180px] rounded-full border border-[var(--line)] bg-white px-3 text-[13px] font-medium text-[var(--ink)] shadow-sm outline-none transition hover:border-[var(--accent)] focus:border-[var(--accent)]"
              >
                <option value="">Tipo do bem: todos</option>
                {tipoBemFiltroOptions.map((option) => (
