...
/** Resposta do backend para `auth/login` e `auth/nwbid` (mesmo formato). */
export type LoginBackendBody = {
  access_token?: string;
  usuario?: {
    nome?: string;
    email?: string;
...
type CookieOptions = { httpOnly: boolean; sameSite: 'lax'; path: string; secure: boolean };

/**
 * Transforma a resposta de login do backend nos cookies de sessão do painel.
 * Usada tanto pelo login por senha quanto pela entrada pelo NWB ID, para que
 * o resto do sistema não precise saber por onde a pessoa entrou.
 */
export function respostaDeSessao(body: LoginBackendBody & { access_token: string }, cookieOptions: CookieOptions): NextResponse {
  const nextResponse = NextResponse.json({
    ok: true,
    empresa_id: body.empresa_atual?.id ?? null,
    termo_pendente: body.termo_pendente === true,
  });
