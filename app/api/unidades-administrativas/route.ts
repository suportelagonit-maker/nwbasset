...
export async function GET() {
  ...
  const response = await fetch(`${API_BASE_URL}/unidades-administrativas?empresa_id=${session.empresaId}&per_page=100`, {
    method: 'GET',
...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${API_BASE_URL}/unidades-administrativas`, {
    method: 'POST',
    headers: {
      ...session.headers,
