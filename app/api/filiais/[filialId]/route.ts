...
export async function PUT(request: Request, context: { params: Promise<{ filialId: string }> }) {
  ...
  const { filialId } = await context.params;
  const payload = await request.json();

  const response = await fetch(`${API_BASE_URL}/filiais/${filialId}`, {
    method: 'PUT',
    headers: {
      ...session.headers,
...
export async function DELETE(_: Request, context: { params: Promise<{ filialId: string }> }) {
  const session = await getSessionHeaders();
